...
def display_price_chart_with_indicators(ticker, info, hist_full):
    ...
    if beta_raw is not None and not (isinstance(beta_raw, float) and np.isnan(beta_raw)):
        st.markdown("### 📉 Beta Pondéré")

        # Calcul du beta pondéré (formule Bloomberg: 2/3 * beta + 1/3 * 1)
        beta_weighted = (2/3 * beta_raw) + (1/3 * 1)

        col_beta1, col_beta2, col_beta3 = st.columns([1, 2, 1])

        with col_beta1:
            st.metric("Beta Brut", f"{beta_raw:.2f}")

        with col_beta2:
            # Jauge visuelle du beta pondéré
            fig_beta = go.Figure(go.Indicator(
                mode="gauge+number",
                value=beta_weighted,
                number={'suffix': '', 'font': {'size': 40}},
                title={'text': "Beta Pondéré", 'font': {'size': 16}},
                gauge={
                    ...
                    'bordercolor': "gray",
                    'steps': [
                        {'range': [0, 0.8], 'color': '#2ecc71'},      # Défensif (vert)
                        {'range': [0.8, 1.2], 'color': '#f1c40f'},    # Neutre (jaune)
                        {'range': [1.2, 1.8], 'color': '#e67e22'},    # Modéré (orange)
                        {'range': [1.8, 2.5], 'color': '#e74c3c'}     # Agressif (rouge)
                    ],
                    'threshold': {
                        'line': {'color': "white", 'width': 3},
                        'thickness': 0.8,
                        'value': 1.0  # Ligne de référence du marché
                    }
                }
            ))
        ...
        with col_beta3:
            st.metric("Beta Pondéré", f"{beta_weighted:.2f}")

            # Interprétation du risque
            if beta_weighted < 0.8:
                st.success("Défensif")
                st.caption("Moins volatil que le marché")
            elif beta_weighted < 1.2:
                st.info("Neutre")
                st.caption("Volatilité proche du marché")
            elif beta_weighted < 1.8:
                st.warning("Modéré")
                st.caption("Plus volatil que le marché")
            else:
                st.error("Agressif")
                st.caption("Très volatil")

        # Note explicative
        with st.expander("ℹ️ Comprendre le Beta Pondéré"):
            st.markdown("""
            **Le Beta Pondéré** (ou Beta Ajusté Bloomberg) est calculé selon la formule:

            `Beta Pondéré = (2/3 × Beta Brut) + (1/3 × 1)`

            Cette pondération ajuste le beta vers 1, car les études montrent que les betas
            tendent à converger vers le marché (beta = 1) sur le long terme.

            **Interprétation:**
            - **Beta < 1**: L'action est moins volatile que le marché (défensif)
            - **Beta = 1**: L'action évolue comme le marché
            - **Beta > 1**: L'action est plus volatile que le marché (agressif)

            **Exemple:** Si le marché baisse de 10%, une action avec un beta de 1.5
            devrait baisser d'environ 15%.
            """)
