...
def calculate_beta(stock_returns, market_returns):
    """Calcule le beta à partir des rendements."""
...
def get_beta_for_period(ticker: str, period: str, benchmark: str = "^GSPC"):
    """Récupère les données et calcule le beta pour une période donnée."""
    try:
        ...
        return beta, len(common_dates)
    except Exception as e:
        return None, 0


def display_weighted_beta_section(ticker: str, info: dict):
    """Affiche l'analyse du Beta pondéré."""
    st.markdown("### ⚖️ Beta Pondéré")

    st.markdown("""
    Le **Beta pondéré** combine les betas calculés sur différentes périodes pour obtenir
    une mesure plus robuste de la volatilité relative d'une action par rapport au marché.
    """)
    ...
    with col_config1:
        ...
        selected_benchmark = st.selectbox(
            ...
            index=0,
            help="Sélectionnez l'indice de marché pour calculer le beta"
        )
    ...
    with col_config2:
        weighting_method = st.selectbox(
            ...
            options=["Standard", "Récent privilégié", "Long terme privilégié", "Personnalisé"],
            index=0,
            help="Choisissez comment pondérer les différentes périodes"
        )

    # Pondérations par défaut selon la méthode
    if weighting_method == "Standard":
        w_1y, w_2y, w_5y = 0.40, 0.35, 0.25
    elif weighting_method == "Récent privilégié":
        w_1y, w_2y, w_5y = 0.60, 0.30, 0.10
    elif weighting_method == "Long terme privilégié":
        w_1y, w_2y, w_5y = 0.20, 0.30, 0.50
    else:  # Personnalisé
        st.markdown("#### Pondérations personnalisées")
        col_w1, col_w2, col_w3 = st.columns(3)
        with col_w1:
            w_1y = st.slider("Poids 1 an (%)", 0, 100, 40) / 100
        with col_w2:
            w_2y = st.slider("Poids 2 ans (%)", 0, 100, 35) / 100
        with col_w3:
            w_5y = st.slider("Poids 5 ans (%)", 0, 100, 25) / 100

        total_weight = w_1y + w_2y + w_5y
        if abs(total_weight - 1.0) > 0.01:
            st.warning(f"⚠️ La somme des poids ({total_weight*100:.0f}%) devrait être égale à 100%")
            # Normaliser
            w_1y, w_2y, w_5y = w_1y/total_weight, w_2y/total_weight, w_5y/total_weight

    # Calcul des betas avec spinner
    with st.spinner("Calcul des betas en cours..."):
        beta_1y, n_1y = get_beta_for_period(ticker, "1y", benchmark_ticker)
        beta_2y, n_2y = get_beta_for_period(ticker, "2y", benchmark_ticker)
        beta_5y, n_5y = get_beta_for_period(ticker, "5y", benchmark_ticker)
    ...
    if beta_1y is not None:
        betas.append(beta_1y)
        weights.append(w_1y)
    if beta_2y is not None:
        betas.append(beta_2y)
        weights.append(w_2y)
    if beta_5y is not None:
        betas.append(beta_5y)
        weights.append(w_5y)

    if not betas:
        st.error("❌ Impossible de calculer le beta. Données insuffisantes.")
        return

    # Normaliser les poids si certains betas sont manquants
    ...
    with col1:
        st.metric(
            "⚖️ Beta Pondéré",
            f"{weighted_beta:.3f}",
            help="Beta combiné pondéré sur plusieurs périodes"
        )

    with col2:
        if beta_1y is not None:
            delta_1y = f"{(beta_1y - weighted_beta):.3f}" if weighted_beta else None
            st.metric("📅 Beta 1 an", f"{beta_1y:.3f}", delta=delta_1y)
        else:
            st.metric("📅 Beta 1 an", "N/A")

    with col3:
        if beta_2y is not None:
            delta_2y = f"{(beta_2y - weighted_beta):.3f}" if weighted_beta else None
            st.metric("📅 Beta 2 ans", f"{beta_2y:.3f}", delta=delta_2y)
        else:
            st.metric("📅 Beta 2 ans", "N/A")

    with col4:
        if beta_5y is not None:
            delta_5y = f"{(beta_5y - weighted_beta):.3f}" if weighted_beta else None
            st.metric("📅 Beta 5 ans", f"{beta_5y:.3f}", delta=delta_5y)
        else:
            st.metric("📅 Beta 5 ans", "N/A")

    # Graphiques
    col_chart1, col_chart2 = st.columns(2)

    with col_chart1:
        # Graphique en barres des betas par période
        fig_bars = go.Figure()
        ...
        if beta_1y is not None:
            periods.append("1 an")
            beta_values.append(beta_1y)
            colors.append('#3498db')
        if beta_2y is not None:
            periods.append("2 ans")
            beta_values.append(beta_2y)
            colors.append('#2ecc71')
        if beta_5y is not None:
            periods.append("5 ans")
            beta_values.append(beta_5y)
        ...
        # Ligne du beta pondéré
        fig_bars.add_hline(
            y=weighted_beta,
            line_dash="dot",
            line_color="orange",
            annotation_text=f"Beta pondéré = {weighted_beta:.3f}",
            annotation_position="left"
        )

        fig_bars.update_layout(
            title="Beta par période",
            xaxis_title="Période",
            yaxis_title="Beta",
            height=350,
            showlegend=False
    ...
    with col_chart2:
        # Graphique en camembert des pondérations
        fig_pie = go.Figure()

        pie_labels = []
        pie_values = []
        pie_colors = ['#3498db', '#2ecc71', '#9b59b6']

        if beta_1y is not None:
            pie_labels.append(f"1 an ({w_1y*100:.0f}%)")
            pie_values.append(weights[0] if len(weights) > 0 else w_1y)
        if beta_2y is not None:
            idx = 1 if beta_1y is not None else 0
            pie_labels.append(f"2 ans ({w_2y*100:.0f}%)")
            pie_values.append(weights[idx] if len(weights) > idx else w_2y)
        if beta_5y is not None:
            idx = len([b for b in [beta_1y, beta_2y] if b is not None])
            pie_labels.append(f"5 ans ({w_5y*100:.0f}%)")
            pie_values.append(weights[idx] if len(weights) > idx else w_5y)
        ...
        fig_pie.update_layout(
            title="Pondération utilisée",
            height=350,
            showlegend=False,
            annotations=[dict(text=f"β={weighted_beta:.2f}", x=0.5, y=0.5, font_size=20, showarrow=False)]
        )
    ...
    st.markdown("---")
    st.markdown("#### 📖 Interprétation du Beta")

    if weighted_beta < 0:
        interpretation = "🔄 **Corrélation négative** : L'action évolue généralement dans le sens inverse du marché."
        risk_level = "Atypique"
        risk_color = "violet"
    elif weighted_beta < 0.5:
        interpretation = "🛡️ **Très défensif** : L'action est beaucoup moins volatile que le marché. Idéale pour les investisseurs prudents."
        risk_level = "Très faible"
        risk_color = "green"
    elif weighted_beta < 0.8:
        interpretation = "🏦 **Défensif** : L'action est moins volatile que le marché. Bon choix pour réduire le risque du portefeuille."
        risk_level = "Faible"
        risk_color = "green"
    elif weighted_beta < 1.0:
        interpretation = "📊 **Légèrement défensif** : L'action suit le marché avec une volatilité légèrement inférieure."
        risk_level = "Modéré-"
        risk_color = "blue"
    elif weighted_beta == 1.0:
        interpretation = "📈 **Neutre** : L'action évolue exactement comme le marché."
        risk_level = "Modéré"
        risk_color = "blue"
    elif weighted_beta < 1.2:
        interpretation = "📈 **Légèrement agressif** : L'action est un peu plus volatile que le marché."
        risk_level = "Modéré+"
        risk_color = "orange"
    elif weighted_beta < 1.5:
        interpretation = "🚀 **Agressif** : L'action amplifie les mouvements du marché. Plus de gains potentiels, mais aussi plus de risque."
        risk_level = "Élevé"
        risk_color = "orange"
    else:
        interpretation = "⚡ **Très agressif** : L'action est très volatile. Risque élevé mais potentiel de gains importants."
    ...
    with col_interp2:
        st.markdown(f"**Niveau de risque:** :{risk_color}[{risk_level}]")
    ...
    with st.expander("📋 Détails du calcul"):
        ...
        if beta_1y is not None:
            contrib_1y = beta_1y * (weights[0] if len(weights) > 0 else w_1y)
            details_data.append({
                "Période": "1 an",
                "Beta": f"{beta_1y:.4f}",
                "Poids": f"{(weights[0] if len(weights) > 0 else w_1y)*100:.1f}%",
                "Contribution": f"{contrib_1y:.4f}",
                "Nb observations": n_1y
            })

        if beta_2y is not None:
            idx = 1 if beta_1y is not None else 0
            contrib_2y = beta_2y * (weights[idx] if len(weights) > idx else w_2y)
            details_data.append({
                "Période": "2 ans",
                "Beta": f"{beta_2y:.4f}",
                "Poids": f"{(weights[idx] if len(weights) > idx else w_2y)*100:.1f}%",
                "Contribution": f"{contrib_2y:.4f}",
                "Nb observations": n_2y
            })

        if beta_5y is not None:
            idx = len([b for b in [beta_1y, beta_2y] if b is not None])
            contrib_5y = beta_5y * (weights[idx] if len(weights) > idx else w_5y)
            details_data.append({
                "Période": "5 ans",
                "Beta": f"{beta_5y:.4f}",
                "Poids": f"{(weights[idx] if len(weights) > idx else w_5y)*100:.1f}%",
                "Contribution": f"{contrib_5y:.4f}",
                "Nb observations": n_5y
            })
        ...
        **Formule utilisée:**
        ...
        ```
        Beta pondéré = Σ (Beta_période × Poids_période)
                     = {' + '.join([f'{b:.3f}×{w:.2f}' for b, w in zip(betas, weights)])}
                     = {weighted_beta:.4f}
        ```

        **Indice de référence:** {selected_benchmark} (`{benchmark_ticker}`)
        """)

    # Avertissements
    with st.expander("⚠️ Limitations du Beta"):
        st.markdown("""
        **Points importants à considérer:**

        - Le beta est **historique** et ne garantit pas le comportement futur
        - Il suppose une relation **linéaire** entre l'action et le marché
        - Le choix de l'**indice de référence** influence fortement le résultat
        - Le beta peut **varier significativement** selon la période choisie
        - Il ne capture pas tous les risques (risque spécifique, liquidité, etc.)
        - Pour les actions peu liquides, le beta peut être **sous-estimé**
        - Les événements exceptionnels peuvent **distordre** le calcul

        **Recommandations:**
        - Utilisez le beta comme **un** indicateur parmi d'autres
        - Comparez avec le beta d'autres entreprises du même secteur
        - Vérifiez la cohérence entre les différentes périodes
        """)
...
def display_historical_analysis(data, info, ticker=None):
    ...
    with st.expander("⚙️ Personnaliser l'affichage", expanded=False):
        ...
        with mod_col11:
            show_weighted_beta = st.checkbox("⚖️ Beta Pondéré", value=True)
        with mod_col12:
            pass
    ...
    if show_weighted_beta:
        tabs_names.append("⚖️ Beta Pondéré")
    ...
    if show_weighted_beta:
        with tabs[tab_idx]:
            display_weighted_beta_section(ticker, info)
        tab_idx += 1
